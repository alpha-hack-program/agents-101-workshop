...
Clica en `rag-pipeline-experiment` y vamos a esperar unos minutos a que las pipelines tengan el status `Succeeded`.

image::experiments-rag-milvus.png[]

¡Genial, nuestros documentos han sido procesados y el modelo puede responder acerca de ellos!

[#53]
=== 5.3 Pipeline de RAGas

Por otro lado, hemos creado *unas pipelines RAGas*. Estas nos permiten hacer pruebas y comprobar la precisión de nuestro RAG pasándole a nuestro modelo una batería de tests.

Vamos a ver los datasets que hemos pasado a la pipeline de RAGas. Vuelve al entorno de desarrollo, el IDE. Echa un vistazo a los *datasets* de la carpeta `llama-stack-demo/materials/datasets`:

image::datasets.png[]

Volvemos a la pestaña de experimentos de OpenShift AI y esta vez clicamos en la pipeline `ragas-scoring-experiment`:
...
Vamos a mirar los pasos que ha seguido el RAGas clicando en la pipeline `ragas-milvus-remote-vector` que tiene el estado `Succeded`:

image::experiments-ragas-milvus.png[]

Aquí tenemos una vista al completo de todos los pasos que ha ejecutado la pipeline RAGas. Ya que hemos visto los resultados del RAGas, el siguiente paso es poner en acción el agente en nuestra propia aplicación.
...
[source,bash,subs="+macros,+attributes"]
----
Basándote en los documentos proporcionados, ¿qué es Lysmark?
----

Debería de ser capaz de dar una respuesta como esta:

image::app-rag-es.png[]
