...
Lo primero que vamos a hacer es clicar en `Projects` en la barra lateral izquierda y seleccionar el proyecto ir a los proyectos de OpenShift AI para elegir dónde crear el Code Server.
...
Vamos a rellenar el formulario de creación del workbench como en la siguiente imagen. En `name`, tienes que escribir *{workbench}* y en `Image server`, selecciona la image de `Code Server | Data Science | CPU | Python 3.12`. Cuando lo tengas listo, confirma al botón inferior izquierdo `Create workbench`.
...
Por último, vamos a logearnos en la línea de comandos de OpenShift para interactuar con nuestro cluster a través del terminal.
