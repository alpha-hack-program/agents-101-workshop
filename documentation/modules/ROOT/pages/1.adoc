...
Para empezar, los facilitadores del taller os daremos acceso al cluster de OpenShift con vuestro usuario y contraseña. Durante el taller, estos os harán falta varias veces.
...
* *Contraseña*: {password}
* *URL*: {openshift-console-url}

A medida que avances con el taller, se te abrirán pestañas en el navegador. Es importante que no las elimines para trabajar fácilmente entre ellas.

¡Clica en la *URL* para acceder al cluster! Introduce el *usuario* y *contraseña* que te han sido proporcionados.
...
Debes de introducir de nuevo tu usuario y contraseña para entrar a la consola de OpenShift AI. Después de hacerlo, estaremos dentro de la consola de OpenShift AI Tu página de inicio debería parecerse a la imagen.
...
Lo primero que vamos a hacer es clicar en `Projects` en la barra lateral izquierda para elegir dónde crear el Code Server.

Elige el proyecto `{namespace}`.

image::project.png[]

Deberías estar a una página como la siguiente. Ahora, clica en `Create a workbench`.

image::project-openshift-ai.png[]

Vamos a rellenar el formulario de creación del workbench como en la siguiente imagen:

* En `name`, tienes que escribir *{workbench}*
* En `Image server`, selecciona la image de `Code Server | Data Science | CPU | Python 3.12`

Cuando lo tengas listo, confirma al botón inferior izquierdo `Create workbench`.
...
Por último, vamos a logearnos en OpenShift a través del terminal para interactuar con el cluster.
...
[source,bash,subs="+macros,+attributes"]
----
oc login -u {username} -p {password} {api-domain}
----

¡Genial! Ya tienes el IDE ya configurado, puedes pasar a la sección **2. Desplegar el Modelo**. 
