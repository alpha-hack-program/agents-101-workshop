* xref:0.adoc[¡Bienvenido al taller!]
** xref:0.adoc#01[Objetivo]
** xref:0.adoc#02[Entorno]

* xref:1.adoc[1. Preparar el Entorno]
** xref:1.adoc#11[1.1 Entrar en OpenShift y OpenShift AI]
** xref:1.adoc#12[1.2 Crear el Code Server]
** xref:1.adoc#13[1.3 Configurar el Code Server]

* xref:2.adoc[2. Desplegar el Modelo]
** xref:2.adoc#21[2.1 ¿Qué es el modelo?]
** xref:2.adoc#22[2.2 Despliegues de OpenShift AI]

* xref:3.adoc[3. Desarrollar el Servidor MCP]
** xref:3.adoc#31[3.1 ¿Qué es MCP?]
** xref:3.adoc#32[3.2 Penalty Engine]

* xref:4.adoc[4. Desplegar la Distribución de Llama Stack]
** xref:4.adoc#41[4.1 ¿Qué es Llama Stack?]
** xref:4.adoc#42[4.2 Desplegar Llama Stack]
** xref:4.adoc#42[4.3 Probar MCP en Playground]

* xref:5.adoc[5. Desplegar la App y Pipelines]
** xref:5.adoc#51[5.1 Desplegar la app y las pipeline RAG, RAGas]
** xref:5.adoc#52[5.2 Pipeline de RAG]
** xref:5.adoc#53[5.3 Pipeline de RAGas]
** xref:5.adoc#54[5.4 Probar la App]

* xref:6.adoc[6. Habilitar Observabilidad]
** xref:6.adoc#61[6.1 Desplegar Observabilidad]

* xref:7.adoc[¡Taller terminado!]
** xref:7.adoc#71[Conclusión]
